...
public class NotePublishCountShardingXxlJob {
    ...
    public void notePublishCountShardingJobHandler() throws Exception {
        ...
        for (int shard = shardRange[0]; shard < shardRange[1]; shard++) {
            ...
            if (selectMapper.selectTempTableExists("t_data_align_note_publish_count_temp_" + tableNameSuffix) == 0) {
            ...
            for (;;) {
                ...
                userIds.forEach(userId -> {
                    // 以笔记表为准重新计算用户发布数
                    long noteTotal = selectMapper.selectCountFromNoteTableByUserId(userId);
