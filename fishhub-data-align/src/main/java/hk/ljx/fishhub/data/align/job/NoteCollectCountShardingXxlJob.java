...
public class NoteCollectCountShardingXxlJob {
    ...
    public void noteCollectCountShardingJobHandler() throws Exception {
        ...
        for (int shard = shardRange[0]; shard < shardRange[1]; shard++) {
            ...
            if (selectMapper.selectTempTableExists("t_data_align_note_collect_count_temp_" + tableNameSuffix) == 0) {
            ...
            for (;;) {
                ...
                noteIds.forEach(noteId -> {
                    // 以收藏关系表为准重新计算笔记收藏数
                    long collectTotal = selectMapper.selectCountFromNoteCollectionTableByUserId(noteId);
