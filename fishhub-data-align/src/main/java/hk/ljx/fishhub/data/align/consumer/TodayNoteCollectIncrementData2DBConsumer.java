...
public class  TodayNoteCollectIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        if (Objects.isNull(collectUnCollectNoteMqDTO)
                || collectUnCollectNoteMqDTO.getNoteId() == null
                || collectUnCollectNoteMqDTO.getNoteCreatorId() == null) {
            throw new IllegalArgumentException("笔记收藏对齐消息缺少业务主键");
        }
        ...
        // ------------------------- 笔记的收藏数变更记录 -------------------------
        String noteDedupKey = RedisKeyConstants.buildDailyNoteCollectNoteIdsDedupKey(date);
        ...
        // ------------------------- 笔记发布者获得的收藏数变更记录 -------------------------
        String userDedupKey = RedisKeyConstants.buildDailyNoteCollectUserIdsDedupKey(date);
