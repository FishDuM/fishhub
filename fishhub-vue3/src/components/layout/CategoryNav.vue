<template>
  <div class="category-nav flex items-center">
    <div class="flex items-center overflow-x-auto py-2 px-4 whitespace-nowrap">
      ...
      <div 
        class="channel"
        :class="{ 'active-channel': activeChannelId === '0' }"
      ...
      <div 
        ...
        :key="channel.id"
        class="channel"
        :class="{ 'active-channel': activeChannelId === String(channel.id) }"
...
const handleChannelClick = (channelId) => {
  // 触发自定义事件，将频道ID传递给父组件
  emit('channel-change', String(channelId))
}
...
@media (max-width: 767px) {
  ...
  .category-nav > div { padding: 8px 12px; }
  .channel { height: 40px; padding: 0 15px; font-size: 15px; }
}
</style>
