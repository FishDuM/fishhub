    ...
    public void retryFailedMessages() {
        ...
        for (MqSendFailureDO failure : failures) {
            ...
            try {
                ...
                log.info("==> 失败 MQ 补发成功, id: {}, topic: {}", failure.getId(), failure.getTopic());
            } catch (Exception e) {
                ...
                mqSendFailureMapper.releaseForRetry(
                        failure.getId(), retryCount, LocalDateTime.now().plusSeconds(delaySeconds), errorMessage);
                log.warn("==> 失败 MQ 补发失败, id: {}, retryCount: {}", failure.getId(), retryCount, e);
            }
        }
    }
}
